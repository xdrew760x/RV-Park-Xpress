...
<section class="section section--builder {{ $background_color }} @if($background_img) text-white @endif"
style="
...
<div class="container flex flex-col md:flex-row md:justify-between">
  ...
  <div class="column {{ $background_state }} {{ $order_state }} {{ $background_color_state }} bg-cover w-full  bg-center {!! $bg_attachment !!} w-full md:{!! $content_width !!}" style=" background-image: url({{ $bg_desktop }}); padding-top: {!! $pad_t !!}px; padding-bottom: {!! $pad_b !!}px; padding-left: {!! $pad_l !!}px; padding-right: {!! $pad_r !!}px;">
    @if( $content )
    <div class="section__content w-full relative @if($margin_value_left) p-15 bg-white border-style-left @endif @if($margin_value_right) bg-white border-style-right @endif @if(! is_admin()) {!! $animate !!} {!! $animate_h !!} {!! $animate_p !!}@endif " style="@if($margin_value_left) left: {!! $margin_value_left !!}px; @endif @if($margin_value_right) right: {!! $margin_value_right !!}px; @endif ">
      {!! $content !!}
    </div>
    @endif
    <!-- end column -->
